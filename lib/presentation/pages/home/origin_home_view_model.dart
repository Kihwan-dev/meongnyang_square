...
class OriginHomeViewModel extends ChangeNotifier {
  OriginHomeViewModel({FirebaseFirestore? firestore, bool oldestFirst = false, int limit = 10})
  ...
  Future<void> _loadInitial() async {
    try {
      final query = _firestore.collection('feeds').orderBy('createdAt', descending: true).orderBy('createdAt', descending: true).limit(_limit);
      ...
      if (feeds.isNotEmpty) {
        final times = feeds.where((e) => e.createdAt != null).map((e) => e.createdAt!).toList();
        if (times.isNotEmpty) {
          // newest
          times.sort();
  ...
  void _subscribeAllRealtime() {
    ...
    final queryRealtime = _firestore.collection('feeds').orderBy('createdAt', descending: true);
  ...
  void _setupRealtimeForNewAdds() {
    ...
    if (_latestCreatedAt == null) {
      final Query<Map<String, dynamic>> firstRealtime = _firestore.collection('feeds').orderBy('createdAt', descending: true).limit(1);
    ...
    Query<Map<String, dynamic>> queryRealtimeAdds = _firestore.collection('feeds').orderBy('createdAt', descending: true);
