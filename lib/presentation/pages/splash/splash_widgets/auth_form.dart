...
class _AuthFormState extends ConsumerState<AuthForm> {
  ...
  //에러메시지 다이얼로그
  void showErrorPopup({String? message, bool showSignUp = false}) {
  ...
  showDialog(
    ...
    builder: (context) {
      return AlertDialog(
        title: const Text('실패!'),
        content: Text((message?.isNotEmpty ?? false) ? message! : '문제가 발생했습니다.'),
        actions: [
          TextButton(
          ...
          if (showSignUp)
            TextButton(
              onPressed: () {
                ref.read(authViewModelProvider.notifier).setMode(false);
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  if (_formKey.currentState!.validate()) {
                    state.isUser ? loginBtnPressed() : joinBtnPressed();
                  }
                },
                child: Text(
                  state.isUser ? 'Login' : 'Join',
