...
class WriteViewModel extends AutoDisposeFamilyNotifier<WriteState, Feed> {
  ...
  Future<String?> autoSaveFeed({
    ...
    try {
      // 수정이 아니라면
      if (arg.authorId != currentUserId) return null;
      // 변한게 없다면
  ...
  Map<String, String> _validateInputs({
    ...
    if (imageData == null && state.isImageChanged) {
      errors["imageData"] = "이미지를 넣어주세요.";
    }

    if (tag.trim().isEmpty && state.isTagChanged) {
      errors['tag'] = '태그를 입력해주세요.';
    }

    if (content.trim().isEmpty && state.isContentChanged) {
